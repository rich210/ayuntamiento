...
/**
 * This is the model class for table "tbl_estado_civil".
 *
 * The followings are the available columns in table 'tbl_estado_civil':
 * @property string $id
...
class EstadoCivil extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('nombre_estado_civil, cancelado, fecha_creacion, fecha_modificacion', 'required'),
			array('cancelado', 'numerical', 'integerOnly'=>true),
			array('nombre_estado_civil', 'length', 'max'=>50),
			array('descripcion', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, nombre_estado_civil, descripcion, cancelado, fecha_creacion, fecha_modificacion', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'tblPersonas' => array(self::HAS_MANY, 'TblPersona', 'estado_civil_id'),
		);
	}
